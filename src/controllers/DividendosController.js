...
export const GetAll = async () => {
  const query = ` SELECT  Id, FundoId, Sigla, Fundo, Cnpj, DtDividendo, Qtd, Valor, Total
                  FROM    vDividendos
...
export const GetItem = async (id) => {
  const query = ` SELECT  Id, FundoId, Sigla, Fundo, Cnpj, DtDividendo, Ano, Mes, Dia, MesNome, Qtd, Valor, Total
                  FROM    vDividendos
                  WHERE   Id = '${id}'`;
  try {
    await pool.connect();
